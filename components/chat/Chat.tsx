                ...
                <div className="max-w-2xl mx-auto space-y-3">
                    {messages.map((m: UIMessage) => (
                        <div key={m.id} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                            <div className={`space-y-1 rounded-lg px-3 py-2 max-w-[80%] ${m.role === 'user' ? 'bg-primary text-primary-foreground' : 'bg-muted'}`}>
                                ...
                                <div className="prose prose-sm dark:prose-invert max-w-none leading-relaxed">
                                    {(() => {
                                        ...
                                        type UIInlinePart = { type: 'text'; text: string } | { type: 'image'; image: string } | { type: string };
                                        ...
                                        if (part.type === 'image') {
                                            const url = (part as { type: 'image'; image: string }).image;
                                            return (
			...
			<form
                ...
                if (!currentChatId) {
                    ...
                    for (const a of attachments) {
                        ...
                        const body = a.textContent && a.textContent.trim().length > 0
                            ? `\n\n${a.textContent}`
                    ...
                    await (sendMessage as unknown as (arg: { content: typeof parts }) => Promise<void>)({ content: parts });
                    setInput("");
                    setAttachments([]);
				}}
                    ...
                    <div className="flex flex-wrap gap-2">
                        {attachments.map((a, i) => {
                            ...
                            if (isImage && a.dataUrl) {
                                return (
                                    <Image key={i} src={a.dataUrl} alt={a.name} width={64} height={64} className="rounded border size-16 object-cover" unoptimized />
                                );
                            }
                            return (
                                <div key={i} className="rounded border px-2 py-1 text-xs bg-muted flex items-center gap-2">
                                    <span className="font-medium truncate max-w-40" title={`${a.name} (${a.type || 'file'})`}>
                                        {a.name}
                                    </span>
                ...
                <div className="flex gap-2">
                    <input
                        ...
                        onChange={async (e) => {
                            ...
                            const readAsText = (file: File) => new Promise<string>((resolve) => {
                                const reader = new FileReader();
                                reader.onload = () => resolve(String(reader.result));
                                reader.readAsText(file);
                            });
                            const newItems: { name: string; type: string; size: number; dataUrl?: string; textContent?: string }[] = [];
                            for (const f of files) {
                                try {
                                    if (f.type.startsWith('image/')) {
                                        newItems.push({ name: f.name, type: f.type, size: f.size, dataUrl: await readAsDataUrl(f) });
                                        continue;
                                    }
                                    if (f.type === 'application/pdf') {
                                        ...
                                        const pdfjs = (await import('pdfjs-dist')) as unknown as {
                                            ...
                                            getDocument: (src: unknown) => { promise: Promise<{ numPages: number; getPage: (n: number) => Promise<{ getTextContent: () => Promise<{ items: Array<{ str?: string }> }> }> }> };
                                        };
                                        try { if (pdfjs.GlobalWorkerOptions) pdfjs.GlobalWorkerOptions.workerSrc = 'https://cdnjs.cloudflare.com/ajax/libs/pdf.js/5.4.149/pdf.worker.min.js'; } catch {}
                                        ...
                                        for (let i = 1; i <= pdf.numPages; i++) {
                                            ...
                                            text += items.map((it) => ('str' in it && typeof it.str === 'string' ? it.str : '')).join(' ') + '\n';
                                        }
                                        newItems.push({ name: f.name, type: f.type, size: f.size, textContent: text });
                                        continue;
                                    }
                                    if (f.type === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document') {
                                        const arrayBuffer = await f.arrayBuffer();
                                        const { value } = await mammoth.extractRawText({ arrayBuffer });
                                        newItems.push({ name: f.name, type: f.type, size: f.size, textContent: value });
                                        continue;
                                    }
                                    if (
                                    ...
                                    const isTextLike =
                                        ...
                                        ['application/json', 'application/x-json', 'application/csv', 'text/csv', 'text/markdown', 'text/html', 'application/xml', 'text/xml'].includes(f.type);
                                    if (isTextLike) {
                                        const text = await readAsText(f);
                                        newItems.push({ name: f.name, type: f.type || 'text/plain', size: f.size, textContent: text });
                                    } else {
                                        // Fallback: store as data URL (binary types like pdf, docx, xlsx)
                                        newItems.push({ name: f.name, type: f.type || 'application/octet-stream', size: f.size, dataUrl: await readAsDataUrl(f) });
                                    }
                                } catch {}
                            }
